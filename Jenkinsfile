pipeline {
    ...
    environment {
        ...
        def CONNECT = 'ssh -o StrictHostKeyChecking=no ubuntu@ec2-3-99-159-231.ca-central-1.compute.amazonaws.com'
    }
    stages {
        ...
        stage('Deploy') {
            steps {
                echo 'Deploying'
                sshagent(['deploy-key']) {
                    sh 'scp -o StrictHostKeyChecking=no -i $SSH_CRED webapp.zip ubuntu@ec2-3-99-159-231.ca-central-1.compute.amazonaws.com:/home/ubuntu'
